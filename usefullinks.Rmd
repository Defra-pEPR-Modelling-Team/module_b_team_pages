...
for (i in seq_along(files)) {
  ...
  space_urls <- na.omit(stringr::str_match(script, "\\[.*?\\)[:space:]"))
  punctuation_urls <- na.omit(stringr::str_match(script, "\\[.*?\\)[:punct:]"))
  end_urls <- na.omit(stringr::str_match(script, "\\[.*?\\)$"))
  
  #remove punctuation
  punctuation_urls <- gsub("\\?","", punctuation_urls)
  punctuation_urls <- gsub("\\.","", punctuation_urls)
  punctuation_urls <- gsub("\\,","", punctuation_urls)
  
  addtl_links <- dplyr::bind_rows(
    data.frame(cbind(title, space_urls)), 
    data.frame(cbind(title, punctuation_urls)),
    data.frame(cbind(title, end_urls))
    ) %>% 
